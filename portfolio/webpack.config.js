...
//__dirname = ............../portfolio ( 프로젝트 경로 )
//path.join(__dirname, 'src') = ...../portfolio/src
...
if(fileSys.existsSync(reduxSaga) && fileSys.existsSync(nodeModules)){
	...
	module.exports = {
		entry: [
			path.join(__dirname, "src", "main", "webapp", "resources", "js", "index") // .../portfolio/src/main/webapp/resources/js/index.js
		],
		mode: "development",
		cache: true,
		devtool: "sourcemaps",
		plugins: [
			...
			new webpack.DefinePlugin({ // 컴파일할 코드에서 특정 문자열을 설정한 값으로 치환.
				...
				"REACT_APP_WEBPACK_MAIN_HOST" : JSON.stringify("http://localhost:8081/portfolio"),
				"REACT_APP_WEBPACK_AUTH_HOST" : JSON.stringify("http://localhost:8082/authserver"),
				"REACT_APP_WEBPACK_RESOURCE_HOST" : JSON.stringify("http://localhost:8083/resourceserver"),
				"REACT_APP_WEBPACK_WEATHER_HOST": JSON.stringify("https://api.openweathermap.org/data/2.5/weather"),
			}),
